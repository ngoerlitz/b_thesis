...
const AP_EL1_RW_EL0_NO: u64 = 0b00 << 6;
const AP_EL1_RW_EL0_RW: u64 = 0b01 << 6;
const AP_EL1_RO_EL0_NO: u64 = 0b10 << 6;
const AP_EL1_RO_EL0_RO: u64 = 0b11 << 6;

// TODO:
// The manual says: "For a translation regime that applies to EL0 and a
// higher Exception level, if the value of the AP[2:1] bits is 0b01,
// permitting write access from EL0, then the PXN field is treated as
// if it has the value 1, regardless of its actual value".
// TODO: Looks like we are forced to actually map the kernel into a different addr. space
// TODO: (we can let the linker script take care of that).
...
const RAM_END: u64 = 0x8000_0000; // 2 GiB (match your `-m`)
...
unsafe extern "C" {
    ...
    static __stack_00_el1_top: u8;
    static __stack_el0_top: u8;
...
fn is_user(pa: u64) -> bool {
    let (start, end) = get_user_text();

    pa >= start && pa < end
}

fn is_read_only(pa: u64) -> bool {
    let (start_rodata, end_rodata) = get_rodata();

    (pa >= start_rodata && pa < end_rodata)
}

fn is_bss(pa: u64) -> bool {
    let (start, end) = get_bss();

    (pa >= start && pa < end)
}

fn is_data(pa: u64) -> bool {
    let (start, end) = get_data();

    (pa >= start && pa < end)
}

fn is_stack_page(pa: u64) -> bool {
    let (start, end) = get_user_text();
    ...
    for l1i in 0..L1_ENTRIES_4G {
        ...
        for l2i in 0..PGTAB_ENTRIES {
            ...
            for l3j in 0..PGTAB_ENTRIES {
                ...
                // if is_mmio(pa) {
                //     L3[l3_idx].0[l3j] =
                //         l3_page_desc(pa, ATTRIDX_DEVICE, SH_IS, AP_EL1_RW_EL0_NO, true, true);
                // } else {
                //     // if is_read_only(pa) {
                //     //     let _ = writeln!(UartSink, "IS_RODATA @ ({l3_idx}, 0, {l3j})");
                //     //
                //     //     L3[l3_idx].0[l3j] =
                //     //         l3_page_desc(pa, ATTRIDX_NORMAL, SH_IS, AP_EL1_RO_EL0_RO, true, true);
                //     // } else if is_bss(pa) {
                //     //     let _ = writeln!(UartSink, "IS_BSS @ ({l3_idx}, 0, {l3j})");
                //     //
                //     //     L3[l3_idx].0[l3j] =
                //     //         l3_page_desc(pa, ATTRIDX_NORMAL, SH_IS, AP_EL1_RW_EL0_RW, true, true);
                //     // } else if is_data(pa) {
                //     //     let _ = writeln!(UartSink, "IS_DATA @ ({l3_idx}, 0, {l3j})");
                //     //
                //     //     L3[l3_idx].0[l3j] =
                //     //         l3_page_desc(pa, ATTRIDX_NORMAL, SH_IS, AP_EL1_RW_EL0_RW, true, true);
                //     // } else if is_stack_page(pa) {
                //     //     let _ = writeln!(UartSink, "IS_STACK @ ({l3_idx}, 0, {l3j})");
                //     //
                //     //     L3[l3_idx].0[l3j] =
                //     //         l3_page_desc(pa, ATTRIDX_NORMAL, SH_IS, AP_EL1_RW_EL0_RW, true, true);
                //     // } else if is_user(pa) {
                //     //     let _ = writeln!(UartSink, "IS_USER @ ({l3_idx}, 0, {l3j})");
                //     //
                //     //     L3[l3_idx].0[l3j] =
                //     //         l3_page_desc(pa, ATTRIDX_NORMAL, SH_IS, AP_EL1_RW_EL0_RW, true, false);
                //     // } else {
                //     //     L3[l3_idx].0[l3j] =
                //     //         l3_page_desc(pa, ATTRIDX_NORMAL, SH_IS, AP_EL1_RW_EL0_NO, false, true);
                //     // }
                //
                //     if is_user(pa) {
                //         let _ = writeln!(UartSink, "IS_USER @ ({l3_idx}, 0, {l3j})");
...
fn get_bss() -> (u64, u64) {
    unsafe {
        let s = &__bss_start as *const _ as u64;
        let e = &__bss_end as *const _ as u64;
...
fn get_data() -> (u64, u64) {
    unsafe {
        let s = &__data_start as *const _ as u64;
        let e = &__data_end as *const _ as u64;
...
fn get_stacks() -> (u64, u64) {
    unsafe {
        let s = &__stack_el0_top as *const _ as u64;
        let e = &__stack_00_el1_top as *const _ as u64;
...
pub unsafe fn init_enable_mmu_4k_l3_identity() {
    ...
    let _ = writeln!(
        UartSink,
        "user_text_start: {:X}, user_text_end: {:X}",
        start, end
    );
    ...
    let _ = writeln!(
        UartSink,
        "__stack_el0_top: {:X}, __stack_00_el1_top: {:X}",
        start, end
    );
    dump_pte_for_va(0xFE20102C);
    dump_pte_for_va(0x28A00);
